    ...
    public function importData($year,$month,$module,$operator,$bigTable,$cecosTable){
        ...
        for($i=1;$i<=intval($month); $i++){
            ...
            foreach ($this->accountArray as $row) {
                ...
                $dml = 'SELECT ROUND(SUM(CAST(DMBTR AS FLOAT64)), 2)*'.strval($row['PAGADO']).' AS SUBTOTAL, '.'"'.strval($year).'"'.' AS ANUALIDAD, '.'"'.$module.'"'.' AS MODULO, '.'"'.$row['ID'].'"'.' AS ID, '.'"'.strval($i).'"'.' AS MES, '.'"'.$row['SUPERCONCEPTO'].'"'.' AS SUPERCONCEPTO, '.'"'.$row['CONCEPTO'].'"'.' AS CONCEPTO '.
